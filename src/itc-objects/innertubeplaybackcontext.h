...
public:
    ...
    InnertubePlaybackContext(bool isContent, const QString& currentUrl, bool autoCaptionsDefaultOn = false, const QString& autonavState = "STATE_ON",
                             ...
                             const QString& referer = "https://www.youtube.com/", int signatureTimestamp = 19193, bool splay = false, int vis = 0)
        : autoCaptionsDefaultOn(autoCaptionsDefaultOn), isContent(isContent), splay(splay), signatureTimestamp(signatureTimestamp),
          vis(vis), autonavState(autonavState), currentUrl(currentUrl), html5Preference(html5Preference), lactMilliseconds(lactMilliseconds),
          referer(referer) {}
